...
- Meters
  - `meter:off` (default): two‑line rows with a thin role‑colored bar directly under each entry.
  - `meter:on`: one‑line rows with a role‑colored background meter behind each entry.
  - Toggle key: `u`.
- Styling
