...
fn clean_number_str(s: &str) -> String {
    ...
    static RE: once_cell::sync::Lazy<Regex> = once_cell::sync::Lazy::new(|| Regex::new(r"[^0-9.+-]").unwrap());
...
fn upper<S: AsRef<str>>(s: S) -> String { s.as_ref().to_uppercase() }

pub fn parse_combat_data(value: &Value) -> Option<(EncounterSummary, Vec<CombatantRow>)> {
    let obj = value.as_object()?;
    if obj.get("type")?.as_str()? != "CombatData" { return None; }

    // Encounter summary
    let enc_obj = obj.get("Encounter").and_then(|v| v.as_object()).cloned().unwrap_or_default();
    let enc_title = enc_obj
        .get("title").or_else(|| get_ci(&enc_obj, "Encounter"))
        .map(val_to_string).unwrap_or_default();
    ...
    let comb = obj.get("Combatant").and_then(|v| v.as_object()).cloned().unwrap_or_default();
    for (name, stats_v) in comb.into_iter() {
        let s = match stats_v.as_object() { Some(s) => s, None => continue };
        // Job filter (party-only via known job codes)
        let job = get_ci(s, "Job").map(val_to_string).unwrap_or_default();
        let job_up = upper(&job);
        if !known_jobs().contains(job_up.as_str()) { continue; }

        let encdps_s = get_ci(s, "encdps").or_else(|| get_ci(s, "ENCDPS")).or_else(|| get_ci(s, "dps"))
            .map(val_to_string).unwrap_or_else(|| "0".into());
        ...
        let dh = get_ci(s, "DirectHitPct")
            ...
            .or_else(|| get_ci(s, "DH%"))
            .map(val_to_string).unwrap_or_default();
        let deaths = get_ci(s, "deaths").or_else(|| get_ci(s, "Deaths"))
            .map(val_to_string).unwrap_or_else(|| "0".into());
